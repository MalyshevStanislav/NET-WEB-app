...
$APPLICATION->SetTitle("AJAX");
   ...
   CJSCore::Init(array('ajax')); /*подключаем библиотеку JS битрикс*/
   $sidAjax = 'testAjax'; /*проверка ajax-запроса*/
if(isset($_REQUEST['ajax_form']) && $_REQUEST['ajax_form'] == $sidAjax){ /*проверяем передан ли параметр ajax_form и сравниваем с $sidAjax*/
   $GLOBALS['APPLICATION']->RestartBuffer(); /*очищаем буфер обмена*/
   echo CUtil::PhpToJSObject(array( /*преобразуем массив в объект с данными для ответа пользователю*/ 
            'RESULT' => 'HELLO',
            'ERROR' => ''
   ));
   die();/*завершаем спринт*/ 
}
